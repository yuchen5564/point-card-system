...
export async function exportLevelsToPdf(activeLevels, baseUrl) {
  ...
  for (let i = 0; i < activeLevels.length; i++) {
    ...
    const taskQrData = await QRCode.toDataURL(taskUrl, { margin: 1, width: 220 })
    const completeQrData = await QRCode.toDataURL(completeUrl, { margin: 1, width: 220 })
    ...
    ctx.fillText('請將此 QR Code 貼在關卡入口', 80, 640)
    ctx.fillText('玩家掃描後可登錄並查看任務詳情', 80, 665)
    ctx.font = '11px monospace'
    wrapText(ctx, taskUrl, 60, 720, 300, 16)
    ...
    ctx.font = 'bold 22px sans-serif'
    ctx.fillText('【 2. 任務完成掃描過關 】', 450, 340)
    ...
    ctx.fillText('⚠ 警示：請由關主/工作人員保管', 460, 640)
    ctx.fillStyle = '#64748b'
    ctx.font = '13px sans-serif'
    ctx.fillText('玩家完成任務後，掃描此碼即代表過關', 460, 665)
    ctx.font = '11px monospace'
    wrapText(ctx, completeUrl, 440, 720, 300, 16)
